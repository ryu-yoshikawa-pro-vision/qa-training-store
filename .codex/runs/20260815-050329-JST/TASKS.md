...
- [x] D1. push後にremote workflow gate、Actions dispatch/watch、artifact download、apply、materialize、Final validationを実行する。
- [x] D2. 実capture後に代表6画面のPNG/WebPを目視確認し、canonical profile以外の画像はpromotionしない。
...
- [x] D6. 同一runの25/25 artifact/APKを検証・apply・promotion・status transitionし、代表6画面を目視確認する。
- [x] D7. materializeとFinal Visual Gate/verify/最新CIの完了を確認し、HANDOFF Bまたはmerge readinessを報告する。
...
- [x] D11. locale patchをユーザーがpushした後、新HEADでState Bのremote gate、dispatch、capture、apply、promotionを再実行する。
...
- [x] D19. 新HEADでprofile normalization、25件capture、artifact検証、promotion、Final Gate、最新CI確認まで完了する。（remote gate確認済み、dispatch待ち）
...
- [x] D28. Run `31875746949`で判明したcapture subflow内部の相対`runFlow` path不整合を最小修正し、local validation後に新HEADで25件captureを再実行する。
