...
- [x] 1. 新しいstrict repair Runを初期化し、開始時branch／PR／HEAD／baselineを確認する
- [x] 2. OTel live path、Hook専用コード、既存testsの参照関係を確定する
- [x] 3. run.jsonのmachine-managed writer／collectorとretroactive finalize経路を確認する
- [x] 4. OTel live pathからHook snapshot処理を除去し、未使用定義だけを整理する
- [x] 5. focused tests、dataset／baseline不変、repository gates、verifyを実行する
- [x] 6. 変更差分、禁止対象、PR本文要件、Run scopeを監査する
- [x] 7. 新repair Runのevaluation／REPORT／TASKSをfinal commit前状態へ更新し、sanitizerを実行する
- [ ] 8. branch safety確認後にcommit・通常pushし、local／remote／PR headを照合する
...
- D1. `collect-run-artifacts`は既存manifestのstatus／validationを保持するcollectorであり、過去Runを再実行なしにretroactive finalizeする専用writer経路は確認できなかった。
