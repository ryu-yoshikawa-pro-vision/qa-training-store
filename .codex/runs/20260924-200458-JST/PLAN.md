...
- Repositoryはproject-scoped MCP serverの固定cwdへ拘束し、`gh api` の `{owner}` / `{repo}` placeholderで解決する。model入力にしない。
...
- GitHub操作は全て `gh api --method GET` に統一し、Repositoryは固定cwdの `{owner}` / `{repo}` placeholderで解決する。
