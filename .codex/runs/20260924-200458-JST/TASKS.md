...
- [ ] 9. official MCP SDK exact versionを確定しdependency / lockfile / project MCP configを更新する。
- [ ] 10. `wait_for_required_ci` MCP serverを実装する。
- [ ] 11. CI waiter contract testとMCP integration testを実装する。
- [ ] 12. implementation harnessとBash / PowerShell verifyをMCP契約へ同期する。
- [ ] 13. focused testとRepository標準verifyを実行する。
- [ ] 14. Run Artifactをfinal commit前状態へ更新し、commit / push / PR本文を同期する。
- [ ] 15. PR #182 latest headで `wait_for_required_ci` を1回callし、実CIの長時間待機を検証する。
...
- なし。次はMCP dependency / config / server実装。
