# Tasks

## Now

- 実行順に並べる（上から順に処理）
- [x] 1. branch、PR #65、origin/main、既存Run、plan、変更scopeを確認する
- [x] 2. 既存Issue #60 planと今回Runの実装計画を更新する
- [x] 3. shell execution stateとGit mutation targetの実装方針を確定する
- [x] 4. Hookへcompound transition、target parser、config、normalization、protected branch guardを実装する
- [x] 5. regression testを追加する
- [x] 6. focused／contracts／format／markdown lint／lint／typecheck／verify／diff checkを実行する
- [x] 7. code-review／repair-loopでself-reviewし、必要ならboundedに修正する
- [x] 8. Run Artifactを更新・sanitizer実行し、commit前確認を行う
- [ ] 9. 通常追加commitを作成し、明示refspecでpushする
- [ ] 10. PR #65本文を最終事実へ更新し、PR metadata／CI状態を確認する
- [ ] 11. REPORT／evaluation／run manifestを最終結果へ同期し完了判定する

## Discovered

- 作業中に発見したタスクはここに追記する（セッション内で増える前提）

## Blocked

- B1. （ブロック時のみ記載）
