...
## Repair decision（PR #14追加指示、Iteration 29）

- `iteration_number`: 29。
- `input_findings`: Android Automationの15 Native Flowと`phase2-native-storefront-cart.yaml`は、`launchApp(clearState: true)`直後に`Scenario Shop`を確認しており、冷起動時の既存Runtime readiness signalを待たないFlowが残っていた。Production Flowも起動直後の固定assertionであり、Production専用の`Scenario Shop`待機契約になっていなかった。iOS BuildはSource側`expo config --json`だけを確認し、生成`.app`内`EXConstants.bundle/app.config`のmetadataを直接検証していなかった。現行文書にもiOS Runtime前提の残差があった。
- `classification`: `must_fix`。起動順序はAndroid実Runtimeのcold-start安定性に、Production待機はTest Control／Harness無効化検証の到達性に、embedded metadataはiOS Artifactの実体保証に直接影響する。文書とContractの不整合も現行保証範囲を誤認させるため同期する。
- `repair_plan`: 全Android Automation Flowで`clearState`後に既存`Native test runtime listening`を30秒待ち、その後に`Scenario Shop`を確認する。Production Flowは`Scenario Shop`だけを30秒待ってからRuntime／Harness非表示を確認する。iOS Automation／Production Build後に生成`.app`の`EXConstants.bundle/app.config`をJSON parseし、`automation / automation / true`または`production / production / false`を検証する。Workflowの順序、埋込みmetadata、Maestro起動順序を最小Contractへ追加し、現行文書をBuild-only方針へ同期する。iOS Runtime／simctl／Maestro／Harness Runtimeは再導入しない。
- `allowed_files`: 変更済みの`maestro/`対象17 Flow、`.github/workflows/native-ios-ci.yml`、`tests/contracts/native-ci-workflow.test.ts`、`tests/contracts/native-test-control-maestro.test.ts`、`README.md`、`docs/PROJECT_CONTEXT.md`、`docs/native/README.md`、`docs/plans/phase2-native-goal/00_master-roadmap.md`、`docs/plans/phase2-native-goal/02_phase2-second-half-purchase-automation.md`、同Run Artifact。
- `changed_files`: `.github/workflows/native-ios-ci.yml`、`README.md`、`docs/PROJECT_CONTEXT.md`、`docs/native/README.md`、`docs/plans/phase2-native-goal/00_master-roadmap.md`、`docs/plans/phase2-native-goal/02_phase2-second-half-purchase-automation.md`、`maestro/native-cart.yaml`、`maestro/native-contract-harness.yaml`、`maestro/native-low-stock.yaml`、`maestro/native-not-found.yaml`、`maestro/native-out-of-stock.yaml`、`maestro/native-payment-retry.yaml`、`maestro/native-production-validation.yaml`、`maestro/native-purchase-limit.yaml`、`maestro/native-purchase.yaml`、`maestro/native-reset-dirty-state.yaml`、`maestro/native-restart-persistence.yaml`、`maestro/native-review.yaml`、`maestro/native-search.yaml`、`maestro/native-session-checkout-restart.yaml`、`maestro/native-storefront.yaml`、`maestro/native-test-control.yaml`、`maestro/phase2-native-storefront-cart.yaml`、`tests/contracts/native-ci-workflow.test.ts`、`tests/contracts/native-test-control-maestro.test.ts`。
- `validation_commands`: `pnpm exec vitest run tests/contracts/native-ci-workflow.test.ts tests/contracts/native-test-control-maestro.test.ts --no-file-parallelism --maxWorkers=1`、`pnpm run test:contracts`、`pnpm run format:check`、`pnpm run lint:markdown`、`pnpm run lint`、`pnpm run typecheck`、`pnpm run test:component:native`、`pnpm run test:repository`、`pnpm run check:native-route-dependencies`、`pnpm run validate:eas:config`、`pnpm run verify`、Maestro-MCP device/run、Android Runbook Doctor／Flow／RuntimeSuite／BoundarySuite、YAML parse、旧起動順序scan、iOS Build-only runtime machinery scan、`git diff --check`、Sanitizer。
- `validation_result`: Focused Contractは2 files／61 tests、全Contractは23 files／173 tests、Format、Markdownlint 176 files／0 issues、Lint 0 errors／64 warnings、Typecheck、Native Component 12 suites／47、Repository 5 files／33、route 38、EAS、`pnpm run verify`（Unit 66／Integration 98／Repository 33／Web Component 76／Native Component 47／Contract 173／Web export 2296）がPASSした。YAMLは21 files parse PASS、旧cold-start patternとiOS Runtime machineryは0 matches。Android Doctor PASS、優先Flow（Test Control、Low Stock、Purchase Limit、Payment Retry、Session Restart）、Production 1/1、Storefront／Cart、RuntimeSuite 5/5、BoundarySuite 5/5をLocal Runbook CLIでPASSした。Production初回はAutomation APKを入れたまま実行したため非表示assertionで停止し、Production APKを明示Install後に1/1へ回復した。Maestro-MCPはDevice Server `UNAVAILABLE`で実行できず、同一端末のLocal Runbook CLIで検証した。
- `remaining_delta`: WindowsにはXcode／`xcodebuild`／`xcrun`／`simctl`／`gh`がなく、iOS Simulator Build、修正HeadのGitHub-hosted Remote Native CI、最新Headの最終`native-ci / verify`は未実行である。iOS Runtime／Maestro／実`expo-sqlite` Harness／Production-validation RuntimeはADR-0011の正式Gate対象外である。Git mutation／pushは行わない。
- `decision`: `continue`。今回のコード、Contract、文書、Android実Runtime、全ローカル品質ゲートは完了したが、iOS Build／Remote確認が未取得のためRunは`partial`／`missing_validation`を維持する。Progress: 97% (37/38)。
