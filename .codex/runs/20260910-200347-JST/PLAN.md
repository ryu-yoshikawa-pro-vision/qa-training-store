...
- `AGENTS.md`と`.codex/templates/TASKS.md`へ、repository working treeのファイル変更を伴う実装・変更タスクだけを対象に、final commit前のtracked Run Artifact確定、push後の最新head必須CI確認、必要なPR本文反映まで完了したCI確認1件を含むProgress算出を追加する。GitHub metadataのみの変更は対象外とし、手動Runの`run.json`制約を機能単位で記録する。
...
- 必須CIは`Web CI`と`Mobile App CI`とし、`Cross Browser Smoke`は通常PR必須CIに含めない。最新PR headで両方successになり、CI結果を記録する必要があるPR本文の更新まで完了するまで完了扱いにしない。
- CI結果はGitHub上のCI結果、PR本文、ユーザー向け最終報告へ記録し、final push後にtracked Run Artifactへ書き戻さない。CI結果をPR本文へ記録する必要がある場合は、その本文更新まで完了してから必須CI確認1件を完了扱いにする。
...
- `run.json`はmachine-managedのため手編集しない。現行collectorはREPORT本文からRun全体の完了状態、手動で実施したvalidation結果、branch/base branchを自動推論しない。`--refresh-git-changed-files`では実行時点のworking tree差分と未追跡ファイルを`changed_files`へ収集できるが、commit済みの過去差分をRun履歴として自動復元しない。今回の実値は`status=pending`、`validation.status=not_run`、`validation.commands=[]`、`branch=null`、`base_branch=null`、`changed_files=[]`であり、REPORTとPR本文へこの機能差を記録する。run.json、collector、manifest仕様は変更しない。
...
- H3: §3でrepository working tree変更タスクだけに必須CI確認1件を加算し、最新headのCI successと必要なPR本文反映までを完了条件に含めれば、tracked task完了とCI成功前またはPR本文更新前のProgress 100%を分離できる。
...
- PLAN -> TASKS -> 実装 -> ローカル検証 -> Run Artifact更新 -> commit -> push -> PR head確認 -> 必須CI確認 -> 必要なPR本文更新 -> ユーザー向け報告の順に進め、CI成功後にtracked Run Artifactへ戻らない。
...
- repository working tree変更タスクのユーザー向けProgressはtracked task数+必須CI確認1件とし、最新headの両CI success確認と必要なPR本文更新が完了する前は100%にしない。CI確認はTASKS checkboxへ戻さない。
...
- `run.json`の`branch=null`、`base_branch=null`、`changed_files=[]`、`validation.status=not_run`、`validation.commands=[]`、`status=pending`を実値として確認する。現行collectorはREPORT本文からRun全体の完了状態、手動validation結果、branch/base branchを自動推論しない一方、`--refresh-git-changed-files`で実行時点のworking tree差分と未追跡ファイルを`changed_files`へ収集でき、commit済みの過去差分をRun履歴として自動復元しない。この機能差を既知事項として記録し、`run.json`、collector、manifest仕様は変更しない。
- 最新headの`Web CI`と`Mobile App CI`がsuccessになり、必要な場合のPR本文更新まで完了した後、PR本文とユーザー向け最終報告へCI結果を記録する。
