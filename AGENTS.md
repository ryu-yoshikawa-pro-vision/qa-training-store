...
- 品質ゲートがFAILした場合、「範囲外」「既存」「baseline」というラベルだけを理由に保留・完了扱いにしてはならない。
- まずbaseline、今回のdiff、shared dependency、test／CI contract、実行環境を確認し、最初の異常と派生エラーを分離する。
- 原因が今回の変更にある、または今回の変更を正しく検証するために必要な場合は`must_fix`として、現在のタスクで最小修正し、関連するゲートを再実行する。
- 原因が独立していても、破壊的操作・権限変更・秘密情報・外部への不可逆な副作用を伴わず、現在の権限内で安全に最小修正できる場合は、単に範囲外という理由でdeferせず修正してゲートを再実行する。
- 安全性・権限・要件判断が必要、破壊的操作が避けられない、または同じfailureが規定回数繰り返される場合だけ停止し、根拠・未実行検証・次の対応をRun Artifactとユーザー向け報告へ記録する。
- 最終報告には、全ゲートのPASS、または上記停止条件に該当する具体的な根拠を必ず記載する。
