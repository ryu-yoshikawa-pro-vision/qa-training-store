...
通常のレビュー出力はfindingsです。findingsがないレビューでも、残るリスクと未検証領域を記載します。永続的なレポートは明示的な依頼またはリポジトリのレビュー結果保存方針が求める場合だけ作成し、具体的な保存先はリポジトリから提供される情報から取得します。このSkillのWorkflowには含めません。
...
- 具体的な保存先、命名、保持期間、active Runのレポートの扱いは、リポジトリのレビュー結果保存方針から提供されます。
