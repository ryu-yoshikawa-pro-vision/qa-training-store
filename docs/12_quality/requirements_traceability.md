...
| CT-BOUNDARY-001 | FR-AR-001～004、NFR-MA-020～023 | bounded-multi-ref | `tests/contracts/architecture.test.ts` — Application / Presentation / Infrastructure / Nativeの技術境界（FR-AR-001、NFR-MA-020～023）；`tests/contracts/image-manifest.test.ts` — `contains every seed reference as a local WebP under the size limit`（FR-AR-002のBuild生成Image Manifest boundary）；`tests/contracts/transactions.test.ts` — `keeps order, payment, shipment, and histories consistent`（FR-AR-003のOrder public DTOにおけるHistory mapping、orderActionVersion、Gateway Key / Version / Actor ID非公開）；`tests/integration/seeds.test.ts` — `resets the database and restores only the seed identities`（FR-AR-004のReset contract） | Request/Command境界、Build Manifest、Order Read DTO、Reset制約 |
