...
| CT-DB-KEY-001 | NFR-RL-011、FR-PR-041/050 | stop | —（Key投影と基本NormalizerのFormal evidenceはあるが、Category / Brand / VariationおよびproductCode / SKUについて、正規化後の重複制約を一貫して確認するCurrent Formal evidenceが欠ける） | Dexieのboolean/null Key投影、名称・Code/SKU正規化、重複制約 |
...
| CT-CATEGORY-002 | FR-PR-055 | stop | —（既存testは末尾sortOrderと並べ替えを確認するが、新規Category作成の同一Transactionを検証する1つのtest titleまたはsuiteはCurrent evidenceにない） | 新規Categoryの末尾sortOrder決定と同一Transaction作成 |
...
| CP-FORM-001 | NFR-AX-001/007、NFR-MA-012 | stop | —（Error Summary / field linkのFormal evidenceはあるが、NFR-MA-012の共有入力上限・文字数制約を確認するCurrent Formal testが見つからない） | 共有入力上限、Error Summary、fieldErrors |
...
| CT-BOUNDARY-001 | FR-AR-001～004、NFR-MA-020～023 | stop | —（Build ManifestとResetの部分的Formal evidenceはあるが、Request / Command補完境界、UI Order DTOの非公開フィールド、NFR-MA-020～023の技術境界を確認する代表Formal suiteがなく、architecture suiteだけでは元の意味全体を確認しない） | Request/Command境界、Build Manifest、Order Read DTO、Reset制約 |
