...
| CT-BOUNDARY-001 | FR-AR-001～004、NFR-MA-020～023 | stop | `FR-AR-003`のみ：`tests/contracts/transactions.test.ts` — `keeps order, payment, shipment, and histories consistent` がOrder public DTOの`orderActionVersion`公開とraw Order `version`、Payment `gatewayIdempotencyKey` / `version`、Shipment `version`、Timeline `actorUserId`非公開をassertする。**不足またはimplementation gap**：`FR-AR-001`（`src/application/contracts/auth.ts`と`src/application/use-cases/auth-use-cases.ts`ではRequestから内部Commandへcontextを補完するが、`tests/contracts/architecture.test.ts`は依存禁止のみでRequest→Commandの補完をassertしない）；`FR-AR-002`（`src/infrastructure/image-assets/static-manifest-repository.ts`はgenerated moduleをRuntime参照しFetchを行わないが、`tests/contracts/image-manifest.test.ts`の`contains every seed reference as a local WebP under the size limit`はManifest内容・local WebPだけでRuntime Fetch非実行をassertしない）；`FR-AR-004`（`src/test-controls/test-control-service.ts`と`e2e/web/fixtures.ts`はResetを実装・運用するが、`tests/integration/seeds.test.ts`の`resets the database and restores only the seed identities`はReset後状態だけで1 Browser Context・1 Page／複数Tab原子性非保証をassertしない）；`NFR-MA-020`（`src/presentation/pages/auth-pages.tsx`のみRHF + Zod、Profile / AddressはRHFのみ、checkout / adminは手動validationでありCurrent implementation gap、既存Formalにも技術使用をassertするtestなし）；`NFR-MA-021`（Native側は`StyleSheet`を使用するがWeb Admin / Layoutは`global.css`と`className`でCSS ModulesなしというCurrent implementation gap、既存FormalにもWeb実装方式のassertionなし）；`NFR-MA-022`（Web実装はReact Aria Componentsを使用するが、既存component testはWidgetの挙動のみでlibrary identityをassertしない）；`NFR-MA-023`（TypeScript型・Enum・Dexie Schemaをcodeに置く実装は確認できるが、codeとMarkdownの正本責務をassertするFormal testなし）。`tests/contracts/architecture.test.ts`のlayer / Native依存境界、`tests/contracts/image-manifest.test.ts`の内容検証、`tests/integration/seeds.test.ts`のReset結果検証だけでは不足部分を代替できない。 | Request/Command境界、Build Manifest、Order Read DTO、Reset制約 |
