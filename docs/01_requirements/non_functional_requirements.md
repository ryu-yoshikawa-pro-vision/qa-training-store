...
| NFR-RL-008 | Gate | 複数Repositoryをまたぐ更新はApplicationTransactionRunnerで1つのDB Transactionに束縛し、部分Commitを残さない |
...
| NFR-MA-005 | Gate | 将来機能の未使用Tableや抽象化をPhase 1に追加しない |
| NFR-MA-006 | Goal | 正本を限定し、同一仕様の文章重複を減らす |
| NFR-MA-007 | Gate | Status表示名、Button文言、Error文言を辞書から参照し、画面内に重複定義しない |
| NFR-MA-008 | Gate | 商品Aggregateの更新境界と在庫調整境界を分離し、既存在庫を商品編集から変更しない |
| NFR-MA-009 | Gate | Category/Brand/Variationの重複判定を共通Normalization関数に集約し、Adapter差を作らない |
...
| NFR-TS-002 | Gate | 固定ID・固定時刻・固定価格をSeed Catalogに定義する |
...
| NFR-TS-007 | Gate | 内部整合性はApplication Integration/Dexie Contractで検証し、E2EのInspectionは固定Read-only DTOに限定する |
...
| NFR-AX-007 | Gate | Error発生時はError SummaryにFocusし、Summary内Linkから該当項目へ移動できる |
...
| NFR-UX-007 | Gate | Toastだけで重要な結果を伝えず、画面内に結果と次のActionを残す |
...
| NFR-SC-007 | Gate | GitHub TokenをBrowser Bundle、Local Storage、IndexedDBに保存せず、ClientからGitHub Contents APIに書き込まない |
...
| NFR-OP-002 | Gate | Cloudflareの固定Automation URLにDeployし、Deployed Smokeを実施する |
| NFR-OP-003 | Gate | Schema変更を含まない場合は直前DeployにRollbackできる |
| NFR-OP-004 | Goal | Release Noteに機能、Seed、Schema、既知制約を記録する |
| NFR-OP-005 | Goal | Native・Public環境は必要になるまで運用対象に追加しない |
