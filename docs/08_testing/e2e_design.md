...
| Project | Gate | 用途 |
|---|---:|---|
| chromium-desktop | PR必須 | Phase 1主要Flow |
| chromium-mobile | main | 360px Storefront主要Flow |
| firefox-desktop / webkit-desktop | 定期・任意 | 互換性確認 |
| deployed-smoke | Deploy後必須 | Cloudflare配信確認 |
...
admin商品登録・公開 → customer購入 → admin発送・配送完了 → customer Review投稿の長大Lifecycleは、PR必須12本とは分離します。

- PR: 独立した必須E2E 12本だけを実行する。
- mainまたは週次: Cross-role Lifecycle 1本を実行する。
- Lifecycle失敗時は各Role区間のTraceを保存し、PR Gateを不必要に不安定化しない。
...
MaestroとNative Flowは`future/phase2`開始時に再設計し、Phase 1 Gateへ含めません。
