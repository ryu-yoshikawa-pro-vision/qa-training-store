...
| GetProductDetailUseCase | 閲覧条件、SKU、Image、未丸めReview Summary・分布を取得し、表示層で平均を小数第1位に丸める |
...
| PrepareProductDuplicateUseCase | active SKU構成・active画像・商品情報を新規登録Form DTOへ転記。Code/SKU空、在庫0。DBに保存しない |
| PreviewProductUseCase | 未保存Form DTOを編集画面内Dialog/Overlay用DTOへ変換しDBに保存しない |
...
| Create/Update/ReorderCategoryUseCase | Createはactive固定で末尾に追加（0件時10、既存時max+10）。Updateは名称、Reorderは全Categoryを10刻みで再採番 |
...
| UpdateCartItemQuantityUseCase | Cart明細を絶対数量に変更。0は削除へ委譲し、親Cart versionを同一Txで更新 |
...
| ExpireCheckoutSessionsUseCase | App起動、Checkout Guard、StartCheckout時に24時間超過をexpiredに変更 |
