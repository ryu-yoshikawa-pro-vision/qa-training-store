...
全Data画面に必要な範囲でLoading、Empty、Success、Validation Error、Operation Error、Conflict、Not Found、Forbiddenを定義します。すべての画面に一律に不要な状態を増やしません。

- Headerに小さなTest Mode Badgeを表示する。
- 実取引なし・実在情報禁止はLogin、Signup、Cart、Checkoutで目立つNoticeとして再掲する。
- 重要な結果はToastだけでなく画面内に残す。
...
- HeroのPrimary Actionは全商品一覧に移動する。
...
- Mobileは価格・在庫・Cart追加をSticky Barに表示する。
...
- `payment_failed`だけ再支払いButtonを表示し、Order詳細の`orderActionVersion`をRequestに渡す。
- `processing?orderId=...`はOrder所有権、Order pending_payment、最新Payment processingを検証して状態確認または再開処理を行い、再支払いButtonを表示しない。
- `delivered`のeligible ItemにReview CTAを表示する。
...
- 商品詳細では平均、件数、1～5別分布、Sort、Pageを表示する。平均は保存済み未丸め値を小数第1位に丸めて表示する。
...
- Bulk結果は成功・失敗件数と対象Linkを画面内に表示する。
...
- Categoryは一覧、追加、編集、並べ替え、有効/無効を提供する。Brandは一覧、追加、編集、有効/無効を提供する。作成時はactive固定とし、Categoryは末尾に追加（0件時10、既存時max+10）、通常編集と状態変更を別Actionにする。
...
- Product関連付けは最大3枚。最初の画像は自動Primary、Primary解除時は先頭に自動再割当する。Primary選択、関連解除、順序変更、Alt Text必須。
...
- Dialog、Popover、Comboboxなど複雑なWeb WidgetはReact Aria Componentsに限定し、独自Headless実装を増やさない。
