...
次の要素を含めて各問題へ回答できれば、このLessonの説明を自己判定できます。

- Regressionが変更の影響範囲を確認する活動であることを説明できる。
- Build、Test、Deploy後Smokeを、対象と失敗時の次の確認先を分けて説明できる。
- ReviewでTest Code、Test Data、Scenario、CI Gateを確認する理由を挙げられる。
- LocalだけでなくPR / main / Manualなどへ実行を接続する理由を説明できる。
- 自分で作成した変更から、実行するTestとその理由を1つ選べる。
