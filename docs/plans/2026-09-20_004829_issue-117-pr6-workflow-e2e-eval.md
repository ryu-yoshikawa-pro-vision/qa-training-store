...
- [ ] Case BのAgent-visible workspaceにはBlack-box用`challenge.json` / `runbook.md`、Instructor material、protected patch、answer key、PR6 answer keyを置かない。Product / Test sourceはGray-boxで参照可能だがoracleとして使わせない。
...
- 現在のPlan branchはlatest `main` `74f6952068ad9b990adbd6a256dd82aeed1d43a7`から2 commit遅れている。追加差分にはSecurity fallback / CI更新とWindows Android Runbook / `android-local.ps1`更新が含まれる。latest `main`でもWindows Local Canonical routeはexplicit physical device serialで、`Doctor`は`-RequirePhysicalDevice`時にphysical deviceを検証する契約を維持している。現時点でPR6の新しい設計blockerは確認していないが、実装開始前にlatest `main`を取り込み、Codex runtime contract、package scripts、CI、Agentic QA、Native、OTel、Skill契約を再確認する。
