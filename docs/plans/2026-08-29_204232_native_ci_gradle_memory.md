...
- 期待成果: 2つのAndroid Release buildへ限定的にGradle JVMメモリ上限を引き上げ、既存build契約を変えずにOOMを解消する。

## 1. ゴール / 完了条件

- ゴール: Android Native CI の Release APK build で発生している Gradle/D8 JVMメモリ不足を、CI build commandの最小変更で解消する。
- 完了条件（PR #83 のDoD）:
  ...
  - `tests/contracts/native-ci-workflow.test.ts` で両jobに同一のJVM memory設定が存在することを既存テスト内で固定する。
  - dependency、Native app source、iOS CI、runner class、Gradle worker数、cache戦略へ変更を広げない。
  - Repository標準validationがPASSする。
  - PR #83 の `Android Automation Build` と `Android Production-validation Build` がPASSし、`OutOfMemoryError` / `Java heap space` が再発しない。
...
- Current understanding:
  ...
  - `tests/contracts/native-ci-workflow.test.ts` は2つのAndroid buildの独立性、実行順、Gradle invocation等を既に文字列契約として検証している。
...
- 仮定してよい細部:
  ...
  - contract testは既存の `keeps Android automation and production builds independent and self-contained` テストへ必要最小限のassertionを追加する。
- 未回答の重要質問: なし。
...
```
  ...
  4. `tests/contracts/native-ci-workflow.test.ts` の既存Android build契約テストに、Automation / Production-validationの両方が `-Dorg.gradle.jvmargs="-Xmx4g -XX:MaxMetaspaceSize=1g"` を含むassertionを追加する。
  5. 4 GiB / 1 GiB適用後も同種OOMが再発した場合は、このPRで追加チューニングを続けず停止してログを再分析する。

- 実行タスク:
  - [ ] 1. 2つのAndroid Release build commandへ同一JVM argsを追加する。
  - [ ] 2. 既存contract testへ2件のassertionを追加する。
  - [ ] 3. 対象外ファイルに差分がないことを確認する。
  - [ ] 4. Repository標準validationを実行する。
  - [ ] 5. PR #83 のAndroid Automation / Production-validation build結果とGradleログを確認する。
...
- 成功判定:
  ...
  - `java.lang.OutOfMemoryError` / `Java heap space` が再発しない。
  - 2jobのGradle invocationに同一の4 GiB heap / 1 GiB metaspace設定が存在する。
...
PR #83をmergeした後、PR #82で組み合わせ確認を行う。

1. PR #82へ最新mainを反映する。
2. PR #82のMobile App CIを再実行する。
3. Expo dependency mismatchが解消された状態で `Native Static`、`Android Automation Build`、`Android Production-validation Build` が成立することを確認する。
4. 問題なければPR #82のmerge判断へ進む。
