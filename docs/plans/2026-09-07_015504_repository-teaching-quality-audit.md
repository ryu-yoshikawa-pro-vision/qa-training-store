...
- ユーザーの継続指示により、同一Runで当初計画を再開する。中間終了時の記録は履歴として保存する。
- 前回以降の差分は監査文書のみ。Product / Specの代表境界、未読Lesson / Workbook、既存Findingの反証を優先し、中間Reportへ追補する。
- 安全な読み取り・診断と監査文書の更新に限定し、実装、Git mutation、Native端末操作、学習者pilotは行わない。

- Living DocumentationはPROJECT_CONTEXTへの短い監査参照とそのhistoryに限定する。提案の採用を意味するADRやProduct仕様の変更は行わない。
