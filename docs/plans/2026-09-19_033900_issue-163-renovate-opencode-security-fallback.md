...
- fail-closedで停止する箇所はActions logへ`needs_human:<固定理由コード>`を出し、外部API失敗では必要に応じてHTTP statusだけを付加する。Alert番号、GHSA ID、dependency名、raw response、credential、tokenを診断ログへ出さない。`workflow_dispatch`のAlert番号はstep `env`へ展開せず、`GITHUB_EVENT_PATH`から`read-alert` / `publish`内で読み取る。Job Summaryの公開結果は従来どおり成功時のPR URLまたは`needs_human`だけとする。
