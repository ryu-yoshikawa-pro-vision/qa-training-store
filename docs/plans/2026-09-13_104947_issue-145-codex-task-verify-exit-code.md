...
- manifestを検証する一時fixtureは、`collect-run-artifacts.ps1`と`collect-run-artifacts.py`を含む実行可能な最小構成を持つ。
- Windows環境で`.ps1`、`.cmd`、`.bat`、command textのtargeted runtime testを実行し、skipではなくPASSを確認する。
...
- 現在の`createWrapperFixture()`が一時repoへコピーしているのは`codex-task.ps1`と`codex-artifact-sanitizer.ps1`であり、`-RecordRunManifest`を実行するためのcollectorはコピーしていない。
...
- Web CIのVitest `contracts` jobは`ubuntu-latest`で実行される。Windows固有の`.ps1`、`.cmd`、`.bat`、command text経路は、このCIだけではruntime検証されない。
...
- manifest fixtureでは`collect-run-artifacts.ps1`と`collect-run-artifacts.py`をproduction sourceから一時repoへコピーする。collector自体は変更しない。
...
- 仮定してよい細部:
  ...
  - 通常のLinux CIでWindows固有runtime testはskipしてよい。ただし、実装完了条件としてWindows環境で同じtargeted testのPASSを確認する。
  - manifest fixtureで必要なPython runtimeが見つからない環境では、その前提を明示してtestをskipできる。ただし、Windowsでの完了確認時はPythonが利用可能な環境でmanifestを含む成功・失敗contractを実行する。
- 未回答の重要質問: なし。
...
- `.github/workflows/ci.yml`
  - contract testは`ubuntu-latest`で実行される。Windows固有runtime経路の完了確認は別途必要。
...
manifest writer、schema、collector、CI workflow、教材、Training関連コードは変更しない。
...
- Web CIのcontract testはLinuxであり、Windows固有runtime経路を実行しない。この不足はCI変更ではなく、Windows環境でのtargeted testを実装完了条件へ追加して補う。
...
4. `report.verify_exit_code`以降のstatus判定、validation更新、manifest writerは変更しない。原因箇所より後段へ配列対策やcastを追加しない。
5. `tests/contracts/codex-task-native-command.test.ts`を拡張し、verify経路のcontractを同じtest体系で確認する。
   - source code contractで`Invoke-VerifyCommand()`の全外部起動経路が`Invoke-NativeCommand()`を使用することを確認する。
   - `Invoke-NativeCommand()`の`CommandArgs`が空配列を許可するcontractも確認する。
   ...
   - command textの失敗fixtureは、command text自身が複数行を出力した後に`exit 7`する。EncodedCommand内部のnative child process終了コード伝播は今回の検証対象にしない。
   - 少なくとも1ケースで標準エラーmarkerも出し、wrapper実行結果から可視であることを確認する。
   - `.ps1`、`.cmd`、`.bat`、`.sh`、default、command textの各経路を、利用可能なruntime上でfixture実行する。通常のcross-platform testでruntimeがない経路はskipし、source code contractで共通helper利用を検証する。
6. manifestを検証するfixtureを既存の`createWrapperFixture()`相当の構造へ追加する。
   - `scripts/codex-task.ps1`
   ...
   - collector source自体には変更を加えない。
   - actual repository Runの`run.json`は編集しない。
7. Windows環境でtargeted contract testを実行し、`.ps1`、`.cmd`、`.bat`、command textのruntime testがskipされずPASSすることを実装完了条件として確認する。
   - Linux CIのPASSだけをWindows固有経路の検証完了とは扱わない。
   - 今回はWindows用GitHub Actions jobを追加しない。
8. 新しいtest helper file、wrapper、dependencyは追加しない。既存fixtureと`spawnSync`、`fs`、`os`、`path`を再利用する。
...
- [ ] 4. manifest fixtureへ`collect-run-artifacts.ps1`と`collect-run-artifacts.py`を含め、`-RecordRunManifest`がproductionと同じ更新経路で動くようにする。
- [ ] 5. verify成功fixtureを追加し、複数行出力でも`verify_exit_code`が単一の`0`になることを自動検証する。
- [ ] 6. verify失敗fixtureを追加し、複数行出力でも指定した非0exit codeが単一値で保持されることを自動検証する。
- [ ] 7. command text失敗fixtureではcommand text自身が`exit 7`することを固定し、今回の修正範囲を越えたexit code伝播仕様変更を避ける。
- [ ] 8. report、validation、manifestの成功・失敗statusを同じruntime contract testで確認する。
- [ ] 9. verify標準出力・標準エラーmarkerがwrapperの出力から確認できることを検証する。
- [ ] 10. default経路で空の`CommandArgs`を渡し、scalar exit codeが返ることをruntime testで確認する。
- [ ] 11. Windows環境で`.ps1`、`.cmd`、`.bat`、command textのtargeted runtime testがskipされずPASSすることを確認する。
- [ ] 12. `.sh`は`bash`が利用可能な環境でruntime testを実行する。
- [ ] 13. targeted contract test、`pnpm run test:contracts`、`pnpm run verify`、`git diff --check`を実行する。
...
Windows環境ではこのcommandを必ず実行し、`.ps1`、`.cmd`、`.bat`、command textのruntime testがskipされずPASSすることを確認する。
...
```text
一時repoにcollect-run-artifacts.ps1が存在する
一時repoにcollect-run-artifacts.pyが存在する
...
- manifest testはcollectorを含む一時fixture上でproductionと同じmachine-managed経路を使用する。
...
- `-RecordRunManifest`のfixtureはcollectorとPython runtimeを必要とする。collectorをコピーせずにmanifest testを作るとverify到達前に失敗するため、fixture構成をPlanどおり固定する。
...
- manifest fixtureのcollectorコピーはtest用fixture構成であり、collector sourceの変更を意味しない。
- Windows固有runtime検証は必須だが、Issue #145のために新しいWindows CI jobは追加しない。
