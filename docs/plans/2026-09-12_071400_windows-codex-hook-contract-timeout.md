...
- Hook matrix等のcontext-sensitive Git commandで発生するHook内部Git context取得
- fixture作成 / cleanup
...
- Windows version / build / architecture、Node、pnpm、`powershell.exe`、`pwsh.exe`、`git`のpath / versionを記録している。
- Vitest側のNode executableとして`process.execPath`、PowerShell launcher側のNode executableとして`Get-Command node`の解決結果を記録し、同一か差があるかを確認している。
- `pwsh.exe`等が存在しない場合はtimeoutではなく環境前提の不足として分離して記録する。
...
現在の環境で再現せず修正根拠を作れない場合は、sourceを推測変更しない。その場合はIssue #142を解決済みとは扱わず、Run Artifactへ次を残して調査を終了する。

- 今回実施した標準runと診断runの回数・生値
- 過去failureと現在の非再現の差
...
PR #127のStrict Runでは、`pnpm run verify`で次の3件が同時に確認されている。

- Windows launcherの5秒timeout 2件
- `executes every common-policy representative from the Hook matrix`の15秒timeout 1件

その後のfocused再実行では、Hook matrixはPASSし、Windows launcherの2件は同じ`5000ms` timeoutで再現した。したがって、3件を1つの原因として扱わず、5秒launcher系と15秒matrix系を分けて切り分ける。

PR #127の変更自体はHook / tracked config / timeoutを変更していないため、直近の再発記録として参照する。正確な実時間と実行順は実装時にRun Artifactから確認できる範囲で確認する。
...
representative Hook matrixでは、context付きcaseを`runNodeHookWithExplicitContexts()`でまとめて処理する一方、contextなしcaseはcaseごとに`runNodeHook()`を起動する。Node起動と、context-sensitive Git commandでのHook内部Git context解決の反復は累積時間増加の候補だが、実測前に原因と断定しない。
...
さらに、#140の2件が渡すcommandは次である。

- `git status --short`
- `rm -f sentinel.txt`
- Python command

現在の`pre_tool_use_policy.mjs`では、これらのcommandは`getGitCommandContext()`の対象にならない。`git status`はread-onlyであり、`rm`とPython commandはGit invocationではない。したがって、#140の2件を調査するときはHook内部の最大4 Git subprocessを主要原因候補へ混ぜず、PowerShell起動、launcher内`git rev-parse`、`Get-Command node`、Node起動、stdin / stdout / stderr、child exitを先に測る。
...
さらにNode Hookがcontext-sensitiveなGit commandを評価すると、`getGitCommandContext()`が追加でGit subprocessを起動する。
...
configured PreToolUse
  ...
  -> pre_tool_use_policy.mjs
     -> context-sensitive Git commandではgetGitCommandContext()
        -> 最大4 Git subprocess
```
...
各Git commandには`2000ms` timeoutがある。これは主にHook matrixやcontext-sensitiveなGit mutation経路の調査対象とする。#140の2件では呼ばれないため、#140とmatrix timeoutの計測結果を混同しない。
...
このrepositoryはconsumer repositoryであるため、通常検証では`-StrictHarness`を付けない。`-StrictHarness`はsource repo maintainer向けの配布前確認である。
...
- PR #127で再現した5秒timeout 2件の正確なtest name、実時間、実行順。
...
- #140について、PowerShell startup、launcher内Git root解決、`Get-Command node`、Node startup、stdin / stdout / stderr、child exitのどれが支配的か。
- matrix timeoutについて、Node startup、Hook内部Git context、case反復、fixture等のどれが支配的か。
...
### 手順1: strict Run、branch、working tree、環境を固定する

このIssueはPreToolUse安全Hook、fail-closed、Git mutation防止契約を変更候補に含むため、実装Runは原則`strict`で扱う。

- 同一タスクのactive Runがあり、workflow levelが`strict`であれば再利用してよい。
- active Runがない、または同一タスクのRunが`strict`ではない場合は新しい`strict` Runを作成する。
- actual `run.json`を直接作成・編集しない。
- source変更範囲は§6の候補から原因に必要な最小範囲へ限定し、RunのTASKS / scope記録へ反映する。
...
`main`がremoteの最新状態を示す保証がない場合は、repository規約に従って最新`main` / merge baseを確認し、そのSHAを記録する。Issue #142作成後にHook関連変更が`main`へ入っていれば、Planの古い前提を更新する。

開始時の`git status --short`に予期しないsource / test / config変更がある場合は、その状態のまま標準性能計測を始めない。既存変更の由来と今回の計測への影響を切り分ける。

`pwsh.exe`等の必要な実行ファイルが存在しない、またはpath解決に失敗する場合は、そのrunをtimeout再現として数えない。configured `pwsh`経路を検証できない環境前提の不足として記録し、必要な検証が未実行ならIssue解決済みと扱わない。
...
- context-sensitive Git commandを含むか
- `getGitCommandContext()`が実行されるか
- 意図的待機の有無
- 過去にtimeout記録があるか
...
historical baseline比較はactive implementation working treeでは行わない。比較用環境は、次のいずれかのように`git rev-parse --show-toplevel`が正常に成立する隔離Git working treeとする。

- `git worktree add --detach <path> <sha>`で作成した一時worktree
- 独立したGit cloneで対象SHAをcheckoutしたworking tree

単純なファイルcopyは使用しない。今回のlauncherとconfigured commandはrepository root解決にGit metadataを必要とするため、`.git`を持たないcopyではbaselineとして成立しない。
...
### 手順5: #140の5秒launcher timeoutを呼び出し単位へ分解する

#140のfocusedまたはfile単体testが再現した場合だけ実施する。
...
#140の2件では`getGitCommandContext()`は実行されないため、まず次へ絞って測る。

- `runWindowsLauncher()` 1回の総時間
- `powershell.exe` startup
...
- stdin copy / close
- stdout / stderr read
- child exit / `WaitForExit`
- fixtureを使う場合だけfixture setup / cleanup

一時計測の値は標準計測と別データとして扱う。`HookResult`、stdout / stderr、exit code、payload contractへ計測値を混ぜない。

### 手順6: Hook matrix timeoutを必要な範囲で分解する

15秒matrix testが再現した場合だけ実施する。#140の2件と同じ原因とは仮定しない。

確認する対象:

- `runNodeHook()` 1回
- `runNodeHookWithExplicitContexts()` batch
- contextあり / contextなしcaseの件数と実行順
- Node startup
- context-sensitive Git commandでの`getGitCommandContext()`呼び出し数
- `getGitCommandContext()`内の各Git command
- fixture作成 / branch設定が関与する場合は`git init` / `git symbolic-ref`

Hook内部Git contextを測る場合は、repository / cwd / `git -C`境界ごとに分ける。#140のlauncher計測結果をmatrixのGit context時間として流用しない。

### 手順7: suite限定の悪化とprocess残存を確認する

focusedでは安定し、file単体 / full contracts / `pnpm run test` / `verify`でのみ悪化する場合だけ実施する。launcher invocation単位計測を機械的に追加せず、先にsuite境界と先行処理との相関を確認する。
...
### 手順8: configured経路を必要な場合だけ計測する

configured経路でのみ悪化する場合、またはdirect経路との差を確認する必要がある場合だけ次を分けて測る。
...
- `Get-Command node`
- Node startup
- context-sensitive Git commandの場合だけNode Hook内部Git context
...
- matrix等のcontext-sensitive Git commandで遅いGit commandと呼び出し回数を特定する。
...
- #140の2件にはこの分岐を適用しない。#140では`getGitCommandContext()`が実行されないためである。
...
ここを修正後検証の正本とする。§1の解決条件、§12の成果物、§13の実装順序では、この節の個別commandを重複定義しない。

### 常に実行するWindows検証

source / test / configを変更した場合は、対象Hook contract fileを3回連続で実行する。
...
この結果はRepository harness / config契約の確認として記録し、Windows Vitest transport検証の代替にはしない。consumer repositoryのため`-StrictHarness`は付けない。
...
- #140の2件だけを根拠にHook内部Git context取得を変更しない。#140のcommandでは`getGitCommandContext()`が実行されないためである。
...
この場合はIssue #142を解決済みとは扱わない。§1の「原因未特定のまま調査を終了する条件」に従って記録する。
...
- 必須検証がPASSしていない場合はIssue解決済みと扱わず、blockerと未達検証をRun Artifactへ記録する。
...
- #140の2件に`getGitCommandContext()`が関与すると誤認すると、実行されていないHook内部Git処理を最適化する方向へ逸れる。
- historical failureの未commit working treeを完全に復元できない可能性がある。
- historical baselineをactive implementation working treeで比較すると、branch state、active Run、一時計測コード、依存状態が混ざる。baseline比較は隔離したGit working treeで行う。
- Git metadataを持たない単純copyではlauncherのroot解決が失敗するため、historical baselineとして使えない。
- baseline比較にはlockfile差分と実行順の影響が混ざる可能性があるため、各baselineの依存を分離し、必要な場合だけ実行順を反転する。
- 開始時のdirty working treeを記録せず計測すると、未commit変更が性能差へ混入する。
...
- Vitest側の`process.execPath`とPowerShell側の`Get-Command node`が異なる場合、Node versionだけでは再現条件を十分に固定できない。
...
- `getGitCommandContext()`はcontext-sensitive Git commandで最大4 Git subprocessを起動するため、matrix timeoutではNode startupだけの計測では不十分である。
...
- Windows version / build / architecture、Node、pnpm、PowerShell、Git、`process.execPath`、launcherが解決するNode pathを含む再現条件
- 必要な実行ファイルが存在しない場合はその事実と未実行経路
- 開始時のbranch / HEAD / merge base / `git status --short`
...
- #140のfocusedまたはfile単体で再現した場合: PowerShell、launcher内Git root解決、Node startup、stdin / stdout / stderr、child exit等の呼び出し単位の計測結果
- 15秒matrixで再現した場合: Node startup、context-sensitive Git case、`getGitCommandContext()`、Git subprocess等の計測結果
- suite限定で再現した場合: suite境界、先行処理、対象PID / 子孫process等の相関結果
- 現行コードで原因を閉じられない場合: 隔離したGit working treeでのhistorical baseline比較
...
1. 同一タスクの`strict` Runを確保し、branch、最新`main`、merge base、開始時working tree、Windows / Node / PowerShell / Git環境情報を固定する。
2. 過去timeout記録と現在のWindows関連test一覧を対応付け、#140の2件では`getGitCommandContext()`が実行されないことを確認する。
3. #140の2件を既定5秒・デフォルトreporterでfocused計測する。
4. file単体 → full contracts → 必要な場合`pnpm run test` → `verify`の順で再現境界を特定する。
5. #140の5秒launcher系と15秒matrix系を別経路として分解する。
6. 現行コードで原因を閉じられない場合だけ、Git metadataを持つ隔離working treeでhistorical baseline比較へ進む。
7. suite限定なら対象実行由来processと先行処理の相関を確認する。
8. configured経路が対象なら外側Git root解決とlauncher内Git root解決を分けて測る。
9. §10の判断基準に従い、原因が確認できた範囲だけ必要最小限の変更を選ぶ。現在非再現で根拠がない場合はsourceを変更しない。
10. 一時計測を削除し、source / test / configを変更した場合は§9の修正後検証を実施する。
11. Windowsでしか検出できない回帰かを基準にfocused Windows CI追加要否を決める。
12. Run Artifact sanitizer、最終diff / status、必要なGitHub Actions結果を確認する。
