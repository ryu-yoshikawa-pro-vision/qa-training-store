...
- 編集場所を自由にすると、評価時にコードや証跡を見つけられない可能性がある。作業場所は自由にし、Part 1からPart 2最終確認まで固定Handoff rootを評価正本として維持する。Training CopyはGitHub Actionsでコードを実行する環境に限定し、別のHandoff rootや評価正本にしない。
- 引き渡しパスを文字列のドット有無で判定すると、spec.tsやcsvを誤拒否したり、親参照を見逃したりする。固定Handoff rootの既知ディレクトリを境界にし、絶対パス、正規化後のルート外、要素完全一致の親参照、ルート外シンボリックリンク、存在性、ファイル種別、スキーマを別々に検証する。Path traversal、root外symlink、schema不正はFAIL、成果物不足はINCOMPLETE、環境は利用可能だが必要な実行がない場合はNOT_RUN、受講者成果と独立した環境要因で評価できない場合はBLOCKEDとする。
...
- Part 1のZIP／任意`part1_distribution_sha`（既存入力名が`source_sha`の場合を含む）、Training Copyの正式な`training_copy_source_sha`、学習者提出`submission_sha`、CI評価`ci_sha`／`execution_sha`を混同すると、どのソースを検証したか追跡できない。各段階のSHAと、prepare／materialize／commit／CI後のworking tree状態を分けて記録する。
- CIの自動RetryをC09の受講者修正後再実行と誤認すると、Failure分析を省略してもPASSできる。`retry_count`と各試行をExecution Receiptへ記録し、別`run_context`の修正後実行と区別する。
- `training:copy:validate`はHEAD／既存の`training-copy-source.json`／workflow契約の確認であり、学習者成果のworking treeをcleanにする検査ではない。prepare直後とHEADが`training_copy_source_sha`のままのmaterialize直後だけに適用し、prepare既知差分、materialize学習者差分、予期しないsource差分を三分類する。commit後の`submission_sha`やCI後の`ci_sha`／`execution_sha`確認とは別に扱い、提出後の`training:copy:validate` PASSを要求しない。
- Execution Receiptを実行後に自動生成しても、その実行のEvidenceを次の実行が削除・上書きするとC09のinitial／repairedを比較できない。正式なReceipt付き実行入口が、既存出力を固定Handoff rootの別参照へ保存してから終了し、run_contextまたは実行識別子の異なるEvidenceを保持することを契約にする。
- C09の初期Failureを学習者の編集済みコードから復元できないと、GitなしのPart 1で診断をやり直せない。提供元の不変assetまたは再抽出可能な教材資材から演習用コピーへ再配置するRecoveryを用意し、完成回答を正本へ書き戻さない。
...
- Part 2の既存Training Copy契約で、事前準備済みのGitHub上Training Copy repository、学習者URL／通常権限、`training:copy:prepare`へ渡す正式な40文字の小文字完全SHA（`training_copy_source_sha`）、`training-copy-source.json`、`training:copy:validate`によるHEAD確認を成立させられない。Part 1の任意`part1_distribution_sha`が不明、Part 1／Part 2のSHA不一致、同一revisionを保証できないことは、この停止条件に含めない。
...
- GitHub Actionsの既存`contents: read`、機密情報なし、既存の成果物順序を守れず、学習者へ通常の書き込み／閲覧を越える追加Permission／Token、Secrets／OIDC、workflow権限設定変更、workflow編集、管理者操作が必要になる。該当ウェーブはT2とし、事前準備済みTraining Copyの正式経路を維持できない具体的理由を記録して停止する。GitHub上のTraining Copy repositoryが用意できない場合は、Commonを止めずP2-01〜P2-03の基礎学習だけをForkで継続し、C12／Training CI／Part 2最終修了をBLOCKEDとする。環境は利用可能だが必要な実行をまだ行っていない場合だけNOT_RUNとする。ForkはC12／Training CIの代替にはしない。
- 第2部（Part 2）の成果物をワークフロー完了後に確定できず、未確定の実行（Run）自身を事前入力にする必要が出た。
- formalなReceipt付き実行入口が既存Playwrightを呼び出しつつrun／case／Retryを分離できない、または後続実行でinitial／repaired Evidenceを保持できない。独立Runner、状態DB、独自URI体系を追加せず、T2を停止する。
- Completion Receiptを`<handoff-root>/completion-receipt.json`へ出力して`receipts/`のExecution Receipt入力へ混入させない構造を成立できない。出力除外の特例で補わず、T1を停止する。
...
| 第2部（Part 2） | Git／GitHub基礎学習でのFork利用と、C12／Training CI・最終修了での正式な`training_copy_source_sha`をHEADにしたTraining Copy、prepare／materialize／commit／CI各段階のデータフロー | 詳細3 |
...
1. Execution Receiptは、テストを実際に実行した後、可能な限り既存Runner／Reporter／実行結果から機械的事実を自動生成する。教材上の正式なReceipt付き実行入口は`training:web:exercise:with-receipt`の1つとし、既存の`training:web:exercise`／`training:web:mobile:exercise`を薄く呼び出してExecution Receipt生成とEvidence保存までを行う。学習者の手書きReceipt、未実行Receipt、意味理解の自動採点、独立Manifest、独自Evidence URI、状態DB、新しい独立Runnerは追加しない。
2. Part 1／Commonの任意`part1_distribution_sha`（既存入力で`source_sha`と呼ばれる場合を含む）は、取得できる場合だけ実際の40文字SHAを記録する。Part 2の正式な40文字の小文字完全SHAは`training_copy_source_sha`として既存の`training:copy:prepare` → `training-copy-source.json` → `training:copy:validate`で独立して確定する。Part 1とPart 2のrevision完全一致は要求しない。
3. Part 2のGit／GitHub基礎学習（P2-01〜P2-03）では学習者自身のForkを利用できる。C12を含むTraining CIとPart 2最終修了は、自己学習開始前に運営側または既存の教材提供手順が準備した、学習者が書き込み可能なTraining Copyを正式経路とする。Fork上のRun／Check／ArtifactをTraining CopyのC12成功証跡へ読み替えず、不要な管理者権限や設定変更を要求しない。Training CopyのGitHub repository自動作成は追加しない。
...
### 9.3 Part 2のTraining Copyに使う正式な`training_copy_source_sha`
...
- `training_copy_source_sha`の形式、Training Copy作成時のHEAD、`sourceSha`／`resolvedSourceSha`。
...
1. Part 1／Commonでは`part1_distribution_sha`を任意項目とし、既存入力で`source_sha`と呼ばれる場合もこの任意参照へ対応付ける。Git管理されたコピーで実際の完全SHAを取得できる場合だけHandoffへ記録する。
...
4. `training:copy:prepare`、既存の`training-copy-source.json`、`training:copy:validate`を正本として、Training CopyのHEADと`training_copy_source_sha`を以降の追跡へ接続する。

Part 1の`part1_distribution_sha`が確認できる場合は、存在する追跡情報としてHandoff等へ保持してよい。確認できない場合は未設定のまま完了でき、架空値・推測値・固定値を生成しない。Part 2では、Part 1のSHAとは独立して正式な40文字の小文字完全SHAを`training_copy_source_sha`として確定し、`training:copy:prepare` → `training-copy-source.json` → `training:copy:validate`を正本としてTraining CopyのHEADとこの正式値を確認する。Part 1のSHAとPart 2のSHAが異なること自体はFAIL、停止、または移行拒否にしない。
...
現行教材の候補にはForkまたは運営側・既存の教材提供手順が準備するTraining Copyがあるが、学習者が本体Repositoryへ直接pushすることは必須にしない。今回の決定では、Git／GitHub基礎学習のbranch／commit／push／PR／reviewには学習者自身のForkを利用できる一方、C12／Training CIとPart 2最終修了には自己学習開始前から準備され、学習者が書き込み可能なTraining Copyを使う。Training workflowは`pull_request`／`workflow_dispatch`を入口とし、workflow tokenは`contents: read`を基本に、Secret、OIDC、Deploy、write権限を要求しない。Fork上のRun／Check／ArtifactをC12の成功証跡へ読み替えず、ForkからTraining Copyへ切り替えられない場合はC12／Part 2最終修了をBLOCKEDとする。環境は利用可能だが必要な実行をまだ行っていない場合だけNOT_RUNとする。
...
学習者へ要求する権限・操作は、基礎学習ではForkまたは書き込み可能なTraining Copyへの通常の書き込み、branch作成、commit、push、PR作成まで、C12／Training CIではTraining Copy上のActions結果・Check・Artifactの閲覧までに限定する。Organization管理者権限、repository管理者権限、Secrets管理、workflow権限設定変更、branch protection変更、GitHub App設定、管理者承認専用操作、workflow編集は必須にしない。必要権限がこの境界を越える、Secrets／OIDC／追加Tokenが必要になる、Training Copyでworkflow契約を満たせない、またはForkをC12の成功経路へ読み替える必要が出た場合は、具体的な制約と影響するT2を記録して停止する。GitHub環境が用意できない場合もCommonを止めず、Part 2をBLOCKEDとして記録する。環境は利用可能だが必要な実行をまだ行っていない場合だけNOT_RUNとする。今回確定した基礎学習のFork利用、C12／Training CIのTraining Copy必須、権限範囲を、改めてOwnerへ質問しない。

### 9.5 その他のW0確認事項

上記3点に加え、次もW0の実測結果と判断をRunへ記録する。

- 固定Handoff rootの`code/`に保持した学習者コードを、C12／Training CIの正式なTraining CopyへRepository相対パスのまま配置する方式と、既存Trainingワークフローの実行対象。例えば`handoff-root/code/training/playwright/exercises/my-cart.spec.ts`はTraining Copyの同じRepository相対パスへ復元し、`implementation_path`、相対import、Case対応を変更しない。ForkはGit／GitHub基礎学習の作業環境としてだけ扱い、Training Copyのsource SHA／validateやC12の成功経路へ接続しない。
...
- Part 2を含むV1でGitHub ActionsのRun／Check／Artifactを実際に一巡できるアカウント／権限／Runnerがあるか。アカウント／権限／Runnerなどの環境がない場合はCommonを止めず、Part 2受入検証だけをBLOCKEDとして記録する。環境は利用可能だが必要な一巡をまだ行っていない場合だけNOT_RUNとして記録する。
...
14. Training Copyは自己学習開始前に準備されたGitHub repositoryであり、学習者へ渡すURL、通常権限、branch／push／PR／Run／Check／Artifact操作の範囲と、repository作成・管理権限を要求しない境界。
15. Part 1からPart 2最終確認までの固定`handoff-root`、Training CopyへmaterializeするWorkbook／Repository相対コード／必要なテキスト成果、GitへcommitしないTrace等のEvidence、CI後にHandoffへ戻すReceipt／Evidenceの流れ。
16. Execution Receiptのrun／case／Retry構造、`training_copy_source_sha`／`submission_sha`／`ci_sha`または`execution_sha`の分離、正式な`training:web:exercise:with-receipt`入口、後続実行でEvidenceを保持する方法。
17. Completion Receiptの出力先、FAIL／INCOMPLETE／NOT_RUN／BLOCKED／PASSの分類、C09のinitial Failure／repaired Pass、Lesson IDによるself-check、CI機械情報とブラウザーEvidenceの確認範囲、Resetの機械確認境界、Gitless診断教材Recovery。
18. active Runの`PLAN.md`／`TASKS.md`／`REPORT.md`が最新契約へ同期され、過去／非active Runとmachine-managedな`run.json`／Hook JSONL／collector情報を直接変更していないこと。
