...
- 人間向け文書の言語と表現は [`WRITING_STANDARDS.md`](WRITING_STANDARDS.md) を正本とする。
...
- `Cross Browser Smoke`はFirefox／WebKitの両方をofficial Playwright container `mcr.microsoft.com/playwright:v1.62.0-noble`内で実行する。packageの`@playwright/test` versionとcontainer image versionは一致させ、Node／pnpmはPhase 1 CIの正本値（Node 24／pnpm 10.34.5）を再利用し、pnpmは`package.json#packageManager`とも一致させる。Phase 1 CIからFirefox／WebKit向け`playwright install --with-deps`は除去し、Cross Browser Smokeではbrowser install step自体を持たない。
...
## Issue #134 Codex Hook quality gate（2026-09-13）
...
- 一般日本語production文章品質は`.textlintrc.json`の5個のtextlint ruleで確定し、`no-unmatched-pair`は技術文書のinline code等を誤検知するため採用しない。`.codex/text-quality-rules.json`はRepository固有custom literal／regex rule用として`not-configured`／空ruleのまま維持する。preset、禁止語、置換表、allowlist、英語混在判定、AIによる意味評価は追加しない。
